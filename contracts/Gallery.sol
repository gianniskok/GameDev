...
contract Gallery is ReentrancyGuard, Stuff {
    ...
    function createGalleryCollection(uint256 _stTimestamp, uint256 _enTimestamp, uint72 _ticketPrice, uint72 _ticketNo , string[] memory _tokenUris, string memory _accessTokenUri, address payable _creator) external  onlyAdmin {
        ...
        newGallery.push(GalleryCollection(_newGallery.current(), _stTimestamp, _enTimestamp, _ticketPrice, _ticketNo, 0, _tokenUris, _accessTokenUri, _creator));
    ...
    function transferFunds(uint256 _galleryId) external payable {
        ...
        uint72 price = newGallery[_galleryId].ticketsSold ;
        creator.transfer(price * 2/10 *10**18);
    }

    function fetchGallery(uint256 _galleryId) external view returns (GalleryItem[] memory){
        require(newItem.length > 0);
